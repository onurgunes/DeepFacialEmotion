...
fname = "egitilmismodel" -- training model name
cmd = torch.CmdLine()
cmd:text()
cmd:text('Train')
cmd:text()
cmd:text('Opt:')
cmd:option('-save', fname, 'fname')
cmd:option('-network', '', 'reload pretrained network') 
cmd:option('-model', 'convnet', 'convnet model')
cmd:option('-full', false, 'use full dataset (50,000 samples)')
cmd:option('-visualize', true, 'visualize')
cmd:option('-seed', 1, 'fixed input seed for repeatable experiments')
cmd:option('-optimization', 'SGD', 'optimization method: SGD | ASGD | CG | LBFGS')
cmd:option('-learningRate', 1e-3, 'learningRate t=0')
...
print('<torch> Thread Number: ' .. opt.threads)
...
----------------------------------------------------------------------
-- get/create dataset
trsize = 1 -- Train total image number (Default)
trainData = {
   size = function() return trsize end
}
trainData = torch.load('trainDataset.t7')
trsize = trainData.data:size(1) -- Train total image number (UPDATED)
...
-- display function: Train GUI
function display2(input)
   iter = iter or 0
   require 'image'
   image.display{image = input, win = winE1, zoom = 2}  -- Show Input
   widgetEgitim.windowTitle = 'DeepFED - User Training'
   widgetEgitim:show()
   if iter % 10 == 0 then
      if opt.model == 'convnet' then
...
function train(dataset, modelAdiKayit)
   ...
   -- do one epoch
   print('<trainer> start train:')
   print("<trainer> epoch number # " .. epoch .. ' [batchSize = ' .. opt.batchSize .. ']')
