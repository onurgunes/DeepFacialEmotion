...
-- classes = {'1imageNotr','2imageMutlu','3imageUzgun','4imageSaskin','5imageSinirli'} yeni
...
function display()
  -- frame = torch.Tensor(100,100)
   ...
   for i=1,faces.size do
      ...
      -- rgbTensorCroped = image.load('/home/mkf/Masaüstü/eclipseLUAproj/DeepFacialEmotion06/test1.png' -- TEST IMAGE EĞİTİMDE KULLANILAN
      ...
      image.display{image = rgbTensorCroped, win = win2, zoom = 1}  -- CROP GOSTER
              ...
              image.display{image = testData.data[1], win = win3, zoom = 1}  -- CROP GOSTER YUV
                    ...
                    for i=1,pred:size(1) do
                      predToplam[i] = predToplam[i] + pred[i]
                      -- print(classes[i], pred[i]) -- HER HESAPLAMA İÇİN YAZ
                    end
                    
                    
                    -- 10 HESAPLAMADA 1 YAZ
                    if predShow == 10 then
                      predToplam[1] = predToplam[1] / (predShow+2)
                      predToplam[2] = predToplam[2] / (predShow+2)
                      predToplam[3] = predToplam[3] / (predShow+2)
                      print(classes[1], predToplam[1])
                      print(classes[2], predToplam[2])
                      print(classes[3], predToplam[3])
                      print('-----------------------------')
                    elseif predShow > 10 then
                      predToplam1 = 0
                      predToplam2 = 0
                      predToplam3 = 0
                      predShow = 0
